    ...
    if (options.indices) {
      createOrUpdateIndices(server, options.indices)
        ...
        .catch(function (error) {
          server.log([plugin.name, 'error'], error);
        });
    }
...
function registerTransformers (server, plugin, transformersMap) {
  ...
  sourceIds.forEach(function (sourceId) {
    ...
    server.app.admin.rawData.db.query('rawData', {
      ...
      .catch(server.log.bind(server, [plugin.name, sourceId, 'error']));
  });
}

function handleDataReport (server, plugin, indicatorName, transform, eventName, report) {
  if (eventName === 'remove') {
    return server.log([plugin.name, report.sourceId, report.scope.id, 'remove'], 'removing integrated data ...');
  }

  if (eventName === 'update') {
    if (report.transformedAt) return;
  }
