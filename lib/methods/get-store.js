...
/**
 * resolve 'raw-data', 'integrated-data' to what is configured in db.*
 */
function toDbName (kazana, name) {
  return {
    'raw-data': kazana.settings.app.db.rawData,
    'integrated-data': kazana.settings.app.db.integratedData
  }[name] || name
}
...
function getAjaxOptions (kazana, options) {
  ...
  if (options.auth === 'admin') {
    return {
      auth: kazana.settings.app.admin
    }
  }
